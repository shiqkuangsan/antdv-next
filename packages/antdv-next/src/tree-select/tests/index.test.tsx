...
describe('tree-select', () => {
    ...
    it('should `treeIcon` work', async () => {
      const wrapper = mount(
        <TreeSelect treeIcon open>
          <TreeNode value="parent 1" title="parent 1" key="p1" icon={<span class="bamboo" />} />
        </TreeSelect>,
        { attachTo: document.body },
      )
      await nextTick()
      expect(document.querySelector('.ant-select-tree-treenode .bamboo')).toBeTruthy()
      wrapper.unmount()
    })
    */
    ...
    it('should support notFoundContent', async () => {
      const wrapper = mount({
        render: () => <TreeSelect treeIcon open notFoundContent="notFoundContent" />,
      }, { attachTo: document.body },
      )
      await nextTick()
  ...
  it('legacy popupClassName', async () => {
    ...
    const wrapper = mount(<TreeSelect popupClassName="legacy" open />, { attachTo: document.body })
    await nextTick()
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `popupClassName` is deprecated. Please use `classNames.popup.root` instead.'),
    )
  ...
  it('legacy dropdownClassName', async () => {
    ...
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const wrapper = mount(<TreeSelect dropdownClassName="legacy" open />, { attachTo: document.body })
    await nextTick()
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `dropdownClassName` is deprecated. Please use `classNames.popup.root` instead.'),
    )
    expect(document.querySelector('.legacy')).toBeTruthy()
  ...
  it('legacy dropdownMatchSelectWidth', () => {
    ...
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    mount(<TreeSelect dropdownMatchSelectWidth open />)
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `dropdownMatchSelectWidth` is deprecated. Please use `popupMatchSelectWidth` instead.'),
    )
  ...
  it('legacy dropdownStyle', async () => {
    ...
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const wrapper = mount(<TreeSelect dropdownStyle={{ color: 'red' }} open />, { attachTo: document.body })
    await nextTick()
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `dropdownStyle` is deprecated. Please use `styles.popup.root` instead.'),
    )
    const dropdown = document.querySelector('.ant-select-dropdown') as HTMLElement
    expect(dropdown).toBeTruthy()
    // Styles checking might be flaky if applied to inner element or overwritten
  ...
  it('legacy dropdownRender', async () => {
    ...
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const wrapper = mount({
      render: () => <TreeSelect dropdownRender={(menu: any) => <div class="custom-dropdown">{menu}</div>} open />,
    }, { attachTo: document.body })
    await nextTick()
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `dropdownRender` is deprecated. Please use `popupRender` instead.'),
    )
  ...
  /*
  it('legacy onDropdownVisibleChange', async () => {
    resetWarned()
    ...
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `onDropdownVisibleChange` is deprecated. Please use `onOpenChange` instead.'),
    )

    await wrapper.find('.ant-select').trigger('mousedown')
    // expect(onDropdownVisibleChange).toHaveBeenCalled(); // We only check warning for legacy props
  ...
  */
  ...
  it('support aria-*', async () => {
    ...
    await nextTick()
    expect(
      document.querySelector('.ant-select-tree-treenode-leaf-last')?.getAttribute('aria-label'),
    ).toBe('label')
    wrapper.unmount()
  })
  */
  ...
  it('deprecate showArrow', () => {
    ...
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining('Warning: [antd: TreeSelect] `showArrow` is deprecated which will be removed in next major version. It will be a default behavior, you can hide it by setting `suffixIcon` to null.'),
    )
  ...
  it('support classNames and styles for basic elements (root, prefix, input, suffix, content)', () => {
    ...
    expect((selectRoot.element as HTMLElement).style.backgroundColor).toBe('rgb(255, 0, 0)')
    expect((prefix.element as HTMLElement).style.color).toBe('rgb(0, 255, 0)')
    expect((content.element as HTMLElement).style.color).toBe('rgb(255, 192, 203)')
  ...
  it('support classNames and styles for placeholder', () => {
    ...
    expect((placeholder.element as HTMLElement).style.color).toBe('rgb(165, 42, 42)')
  })
  ...
  it('support classNames and styles for items (item, itemContent, itemRemove)', async () => {
    ...
    const popup = document.querySelector('.ant-tree-select-dropdown') as HTMLElement
    ...
    expect((selectedItem.element as HTMLElement).style.color).toBe('rgb(70, 130, 180)')
    expect((itemContent.element as HTMLElement).style.color).toBe('rgb(255, 20, 147)')
    expect((itemRemove.element as HTMLElement).style.color).toBe('rgb(34, 139, 34)')

    const itemTitle = document.querySelector('.ant-select-tree-title') as HTMLElement
    if (itemTitle) {
      expect(itemTitle.classList.contains(customClassNames.popup.itemTitle)).toBeTruthy()
  ...
  it('treeSelect ContextIsolator', async () => {
    ...
    // expect(compactButton.classes()).toContain('compact') // This might differ in Vue implementation
  ...
  it('should support switcherIcon from ConfigProvider', async () => {
    const wrapper = mount({
      render() {
        return (
          ...
          >
            <TreeSelect open treeDefaultExpandAll>
              <TreeNode value="parent 1" title="parent 1" key="0-1">
                <TreeNode value="parent 1-0" title="parent 1-0" key="0-1-1">
                  <TreeNode value="leaf1" title="my leaf" key="random" />
                  <TreeNode value="leaf2" title="your leaf" key="random1" />
    ...
    const customIcon = document.querySelector('[data-testid="custom-expanded"]') || document.querySelector('[data-testid="custom-collapsed"]')
